...
use crate::{
    ...
    game_templates::{TemplateContext, TemplateManager},
...
impl CliHandler {
    ...
    pub async fn handle(&mut self, cli: Cli) -> Result<()> {
        ...
        match cli.command {
            Commands::Create {
                ...
                self.handle_create(description, model, name, output, template, in_place)
                    .await
            }
            Commands::Add {
                feature,
            ...
            Commands::Build { operation } => self.handle_build_operation(operation).await,
            Commands::Project { operation } => self.handle_project_operation(operation).await,
            Commands::Template { operation } => self.handle_template_operation(operation).await,
        }
    }
    ...
    async fn handle_create(
        ...
        let agent = self.agent.as_ref().ok_or_else(|| {
            BevyAIError::ai_api("No AI agent available. Please configure API keys.".to_string())
        })?;
        ...
        info!(
            "Creating game '{}' from description: {}",
    ...
    async fn handle_improve(
        ...
        if !file_path.exists() {
            return Err(BevyAIError::file_operation(
                "read",
    ...
    async fn handle_init(
        ...
        info!("Project location: {}", project_path.display());
        info!(
            "To get started: cd {} && bevy-agent add \"your first feature\"",
    ...
    async fn handle_config(
        ...
        if show {
            ...
            println!(
                "Anthropic: {}",
                if self.config.anthropic.is_some() {
                    "Configured"
                } else {
                    "Not configured"
                }
        ...
        match operation {
            ProjectOperation::Info { project } => {
                ...
                if let Some(config) = manager.config() {
                    ...
                    println!(
                        "Updated: {}",
                        config.metadata.updated_at.format("%Y-%m-%d %H:%M:%S")
                    );
