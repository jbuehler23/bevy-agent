...
use bevy_agent::config::{AnthropicConfig, GoogleConfig, OpenAIConfig};
use bevy_agent::{AIConfig, BevyAIAgent, Result};
...
fn create_openai_config() -> Result<AIConfig> {
    let api_key = std::env::var("OPENAI_API_KEY").map_err(|_| {
        bevy_agent::BevyAIError::Io(std::io::Error::new(
...
async fn test_provider(config: AIConfig, provider_name: &str) -> Result<()> {
    ...
    match request.execute().await {
        ...
        Err(e) => {
            println!("{provider_name} failed: {e}\n");
        }
    }
